...
public class InterledgerPaymentWithPskCodecTests {
  ...
  public static Collection<Object[]> data() {
    ...
    // data. A nonce is used to generate the sharedSecretKey, so that the same private headers
    // will not produce the same ciphertext. Also note that the nonce's inclusion in the ILP
    // payment ensures that multiple payments using the same shared secret result in different
    // hashes.
