...
import org.interledger.codecs.btp.BilateralTransferProtocolPrepareCodec;
 ...
 * BilateralTransferProtocolPrepare}.</p>
 *
 * @see "http://www.oss.com/asn1/resources/books-whitepapers-pubs/Overview%20of%20OER.pdf"
 */
public class BilateralTransferProtocolPrepareOerCodec implements
    BilateralTransferProtocolPrepareCodec{

  @Override
  public BilateralTransferProtocolPrepare read(final CodecContext context, final InputStream inputStream)
      throws IOException {
    ...
    final byte[] condition = context.read(OerUint256.class, inputStream).getValue();

    final ZonedDateTime expiresAt = context.read(OerGeneralizedTime.class, inputStream).getValue();

    final List<SubProtocolData> data = context.read(OerSequenceOfSubProtocolData.class, inputStream)
        .getSubProtocols();

    return BilateralTransferProtocolPrepare.builder()
        .requestId(requestId)
        .transferId(transferId)
        .amount(amount)
        .condition(Condition.of(condition))
        .expiresAt(expiresAt)
  ...
  @Override
  public void write(final CodecContext context, final BilateralTransferProtocolPrepare instance,
    ...
    context.write(OerUint256.class, new OerUint256(instance.getExecutionCondition().getHash()), outputStream);

    // 4. Write the expiresAt portion of the packet.
    context.write(OerGeneralizedTime.class, new OerGeneralizedTime(instance.getExpiresAt()), outputStream);
