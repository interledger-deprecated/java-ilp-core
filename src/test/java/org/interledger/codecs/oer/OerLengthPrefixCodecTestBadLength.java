...
public class OerLengthPrefixCodecTestBadLength {
  ...
  public void test_BadLengthIndicator() throws IOException {
    ...
    CodecContext context = new CodecContext()
        .register(OerLengthPrefix.class, codec);
    
    /* we create an incorrect length indicator that says that the next two bytes encode the 
     * length of the actual data. however, we'll only supply one byte of information. the 
     * codec should detect this and fail, since it cannot read the length indicator. */
