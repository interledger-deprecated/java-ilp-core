 ...
 */
public enum BilateralTransferProtocolErrorTypes {
  ...
  BilateralTransferProtocolErrorTypes(String code, String name) {
    this.code = code;
    this.name = name;
  }
  ...
  public static BilateralTransferProtocolErrorTypes fromCode(String code) {
    if(T00_UNREACHABLE.code.equalsIgnoreCase(code)) return T00_UNREACHABLE;
    if(F00_NOT_ACCEPTED.code.equalsIgnoreCase(code)) return F00_NOT_ACCEPTED;
    if(F01_INVALID_FIELDS.code.equalsIgnoreCase(code)) return F01_INVALID_FIELDS;
    if(F03_TRANSFER_NOT_FOUND.code.equalsIgnoreCase(code)) return F03_TRANSFER_NOT_FOUND;
    if(F04_INVALID_FULFILLMENT.code.equalsIgnoreCase(code)) return F04_INVALID_FULFILLMENT;
    if(F05_DUPLICATE_ID.code.equalsIgnoreCase(code)) return F05_DUPLICATE_ID;
    if(F06_ALREADY_ROLLED_BACK.code.equalsIgnoreCase(code)) return F06_ALREADY_ROLLED_BACK;
    if(F07_ALREADY_FULFILLED.code.equalsIgnoreCase(code)) return F07_ALREADY_FULFILLED;
    if(F08_INSUFFICIENT_BALANCE.code.equalsIgnoreCase(code)) return F08_INSUFFICIENT_BALANCE;
