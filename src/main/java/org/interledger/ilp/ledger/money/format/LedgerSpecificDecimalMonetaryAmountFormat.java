package org.interledger.ilp.ledger.money.format;
...
/**
 * A formatter for String amounts that considers the scale and precision of the ledger that gives context to the amount
 * 
...
public class LedgerSpecificDecimalMonetaryAmountFormat implements MonetaryAmountFormat {
  ...
  public MonetaryAmount parse(CharSequence text) throws MonetaryParseException {
      ...
      try {
          ...
          return Monetary.getDefaultAmountFactory().setCurrency(currencyUnit).setNumber(number).create();
  ...
  public String toString() {
      ...
      sb.append(LedgerSpecificDecimalMonetaryAmountFormat.class.getName()).append('{')
              ...
              .append(" precision: ").append(this.context.getInt(KEY_PRECISION)).append(',')
              .append(" scale: ").append(this.context.getInt(KEY_SCALE)).append(',')
